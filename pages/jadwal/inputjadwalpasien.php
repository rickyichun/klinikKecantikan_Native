        ...
        <div class="content-wrapper">
            ...
            <div class="card">
                <div class="card-header">
                    <h3 class="card-title">Tambah Jadwal Pasien Lama</h3>
                </div>
                <!-- /.card-header -->
                <!-- form start -->
                <div class="card-body">
                    <a href="inputjadwal.php">
                        <button class="btn btn-sm btn-primary"><i class="fa fa-user"></i> Pasien Baru</button>
                    </a>
                </div>
                <form enctype="multipart/form-data" action="../control/insert.php?mode=9" method="post">
                    <div class="card-body">
                        <div class="form-group">
                            <label>Nama Pasien</label>
                            <select class="form-control selectize" id="idpas" name="idpas">
                                <?php					
                                    $tampil=mysqli_query($conn, "SELECT * from pasien order by id desc");
                                    while($data=mysqli_fetch_array($tampil)){
                                ?>
                                <option value="<?php echo $data['id'];?>"><?php echo $data['nama'];?></option>
                                <?php } ?>
                            </select>
                            <input type="hidden" class="form-control" id="nama" name="nama" value="">
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">Tanggal Datang</label>
                            <input type="date" class="form-control" value="<?php echo date('Y-m-d'); ?>" id="tgldtg"
                                name="tgldtg" placeholder="Tanggal Datang" required>
                        </div>
                        <div class="form-group">
                            <label for="exampleInputEmail1">DP</label>
                            <input type="number" class="form-control" id="dp" name="dp" placeholder="Nominal DP"
                                required>
                        </div>
                        <div class="form-group">
                            <label>Nama Marketing</label>
                            <select class="form-control selectize" id="mkt" name="mkt">
                                <?php					
