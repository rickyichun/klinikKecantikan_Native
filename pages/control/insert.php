...
switch ($mode) {
	...
	case 10:
		...
		if($noreq!='tnp_request'){
			$query4 = mysqli_query($conn, "UPDATE reqbarang SET status='done_qty', tglupdate='$timestamp' WHERE id='$noreq'");
		}
	...
	case 14:
		...
		$query = mysqli_query($conn, "UPDATE riw_trx SET status='done', tglupdate='$timestamp' WHERE notrx='$notrx'");
		if($idjdwl!=0){
		...
		//simpanLog
			$keter = $fullname." menyelesaikan pembayaran dengan Notrx ".$notrx ; 
			$querylog = mysqli_query($conn, "INSERT INTO daf_log (keterangan, iduser, tanggal)
			VALUES ('$keter','$iduser','$timestamp')");
	...
	case 17 :
		...
		if ($cekbulan>0){
			...
			$gp 		= $datah['gapok'];
				//Komisi Transaksi
				$query3 = mysqli_query($conn, "SELECT * FROM riw_trx, perawat, namapkt, jenispkt WHERE perawat.id='$idpwt' AND riw_trx.idpwt=perawat.id AND riw_trx.idpkt=namapkt.id AND namapkt.idjenis=jenispkt.id AND month(riw_trx.tgl)='$bulangaji' AND riw_trx.status='done_brg'");
				$cekfee = mysqli_num_rows($query3);
				if($cekfee>0){
					while($data3 = mysqli_fetch_array($query3)){
						$qtypkt[]=$data3['qtypkt'];
						$notrxriw[]=$data3['notrx'];
						$idjns[]=$data3['idjenis'];
			...
			if($cekfee!=0) {
				for($i=0; $i<$cekfee; $i++) {
					$queryyy=mysqli_query($conn, "INSERT INTO det_komisi (notrx, nogaji, idjnspkt, qty) VALUES ('$notrxriw[$i]','$notrx','$idjns[$i]','$qtypkt[$i]')");
				}
			}
	...
	case 24 :
		...
		for($i = 0;$i < count($idbrg);$i++) {
			//ambil barang paket
			$idbrgarr = $idbrg[$i];
			$query3 = mysqli_query($conn, "SELECT * FROM m_barang WHERE id='$idbrgarr'");
			$data3  = mysqli_fetch_array($query3);
			$hasil[] = array(
				"id"=>$data3['id'],
				"stok"=>$data3['stok']-$qty[$i],
			);
			if($hasil[$i]['stok'] < 0){
				$status = false;
				$brgminus[] = $data3['namabrg']; 
			} else {
				// update stok barang paket
				$hasilqty = $hasil[$i]['stok'];
				$idbrgakhir = $hasil[$i]['id'];
			}
		}
		...
		if($status == false){
			echo "<script>window.history.go(-1)</script>";
			$_SESSION['alert'] = "warning"; 
			$_SESSION['pesan'] = "Stok <b>".implode(", ", $brgminus)."</b> tidak mencukupi!"; 
		} else{
			
			$query4 = mysqli_query($conn, "UPDATE m_barang SET stok='$hasilqty', tglupdate='$timestamp' WHERE id='$idbrgakhir'");
			$query5 = mysqli_query($conn, "UPDATE riw_trx SET status='done_brg' WHERE id='$idtrx'");
			for($i = 0;$i < count($idbrg);$i++) {
				$query=mysqli_query($conn, "INSERT INTO riw_brgkeluar ( idtrx, idbarang, qtyout, iduser, tglupdate) VALUES ('$idtrx','$idbrg[$i]','$qty[$i]','$iduser', '$timestamp')");
			}
			if ($query){
				echo "<script>window.location = '../transaksi/history_trx.php'</script>";
				$_SESSION['alert'] = "berhasil";
				$_SESSION['pesan'] = "Input pemakaian barang berhasil!";	
			} else {
				echo "<script>window.location = ../transaksi/history_trx.php'</script>";	
				$_SESSION['alert'] = "gagal";
				$_SESSION['pesan'] = "Input pemakaian barang gagal!";	
			}
		}
